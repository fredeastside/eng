...
$absitem = new fmakeMeets();
$absitem->setId($request->id);
$absitem->tree = false;

$meets_categories_obj = new fmakeMeetCategories();
$meets_categories = $meets_categories_obj->getAll();
...
switch ($request->action) {
    ...
    case 'new': // Далее форма
        ...
        $_select = $form->addSelect("Категория", "id_meet");
        //$_select->AddOption(new selectOption("", "", false));
        foreach($meets_categories as $category){
            $_select->AddOption(new selectOption($category['id'], $category['title'], (($category['id'] == $items['id_meet'] || ($request->action=='new' && $file=='mod_text') )? true : false )));
        }
        ...
        $form->addCheckBox("Главное событие", "in_main", $items["main"], true);
