...
$absitem = new fmakeIncidents();
$absitem->setId($request->id);
$absitem->tree = false;

$incident_categories_obj = new fmakeIncidentsCategories();
$incident_categories = $incident_categories_obj->getAll();
//printAr($incident_categories);
...
switch ($request->action) {
    ...
    case 'new': // Далее форма
        ...
        $form = new utlFormEngine($modul, "/admin/index.php?modul=" . $request->modul, "POST", "");
        ...
        $_select = $form->addSelect("Категория", "id_incident");
        //$_select->AddOption(new selectOption("", "", false));
        foreach($incident_categories as $category){
            $_select->AddOption(new selectOption($category['id'], $category['title'], (($category['id'] == $items['id_incident'] || ($request->action=='new' && $file=='mod_text') )? true : false )));
        }
