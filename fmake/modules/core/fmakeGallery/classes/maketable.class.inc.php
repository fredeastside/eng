...
class MakeTable {
	...
	function create($fieldsArray, $fieldHeadersArray=array()) {
		if (is_array($fieldsArray)) {
			...
			$table= "\n".'<table'. ($this->tableWidth ? ' width="'.$this->tableWidth.'"' : ''). ($this->tableClass ? ' class="'.$this->tableClass.'"' : '')." cellspacing=\"0\" cellpadding=\"0\">\n". ($header ? "\t<tr class=\"".$this->rowHeaderClass."\">\n".$header."\t</tr>\n" : '').$table."</table>\n";
			if ($this->formElementType) {
				$table= "\n".'<form id="'.$this->formName.'" name="'.$this->formName.'" action="'.$this->formAction.'" method="POST">'.$table;
			}
			if ($this->pageNav) {
				$table .= '<div id="max-display-records" class="clearfix"><select style="display:inline" onchange="javascript:updatePageSize(this[this.selectedIndex].value);">';
				...
				$table .= '<div id="pagination" class="clearfix">Go to page: <ul>'.$this->pageNav.'</ul></div>';
				$table .= '<script language="javascript">function updatePageSize(size){window.location = \''.$_SERVER['REQUEST_URI']. (strpos($_SERVER['REQUEST_URI'], '?') ? '&' : '?').'pageSize=\'+size;}</script>';
			}
			if ($this->allOption) {
				$table .= '
